...
<script>
  ...
  let {
    ...
    minRadius = 3,
    maxRadius = 50,
    /**Toggle triangle size scaling like a gangsta*/
    scaleRadiiByZoom = true,
    radiusScaleStrength = 0.99,
  } = $props()
  ...
  // sign > 0 → triangle points up; sign < 0 → triangle points down
  function trianglePoints(cx, cy, r, sign) {
    const widthScale = 0.75
    const heightScale = 1.25
    const halfBase = r * (Math.sqrt(3) / 2) * widthScale
    const half = r / 2
    const baseY = sign < 0 ? cy - half : cy + half
    const apexY = sign < 0 ? baseY + 1.5 * r * heightScale : baseY - 1.5 * r * heightScale
  ...
  function update() {
    ...
    projectedPoints = points.map((p) => {
      ...
      // const baseR = Number.isFinite(value) ? radiusScale(magnitude) : minRadius
      const scaledR = Number.isFinite(value) ? radiusScale(magnitude) : minRadius
const baseR = minRadius + (scaledR - minRadius) * radiusScaleStrength
      const sign = Number.isFinite(value) && value < 0 ? -1 : 1
  ...
  $effect(() => {
    ...
    void [points, radiusKey, minRadius, maxRadius, scaleRadiiByZoom, radiusScaleStrength]
    update()
  })
</script>
...
>
  ...
  <div class="map-stage">
    ...
    {#each projectedPoints as point}
      <polygon
        points={trianglePoints(point.x, point.y, point.r, point.sign)}
        fill={point.fill ?? "red"}
        stroke="black"
        stroke-width="1"
        opacity="1"
